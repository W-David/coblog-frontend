...
!(function () {
  ...
  let t
  function o(o, s, i) {
    if (!o || (sessionStorage.getItem('waifu-text') && sessionStorage.getItem('waifu-text') > i)) return
    t && (clearTimeout(t), (t = null)), (o = e(o)), sessionStorage.setItem('waifu-text', i)
  ...
  class s {
    ...
    async loadModel(t, s, i) {
      if (
        (localStorage.setItem('modelId', t), localStorage.setItem('modelTexturesId', s), o(i, 4e3, 10), this.useCDN)
      ) {
  ...
  function n(t) {
    const n = new s(t),
      c = t.tools && Array.isArray(t.tools) && t.tools.length > 0
    localStorage.removeItem('waifu-display'), sessionStorage.removeItem('waifu-text')
    const a = c
      ...
      window.addEventListener('mousemove', () => (i = !0)),
          ...
          (function (e) {
            ...
            if ('' !== document.referrer) {
              ...
              return location.hostname === e.hostname
                ? t
                : ((o = s in i ? i[s] : e.hostname), `Hello！来自 <span>${o}</span> 的朋友<br>${t}`)
            }
            return t
          })(t.time),
          7e3,
        ...
        window.addEventListener('mouseover', s => {
          for (let { selector: i, text: n } of t.mouseover)
            if (s.target.matches(i)) return (n = e(n)), (n = n.replace('{text}', s.target.innerText)), void o(n, 4e3, 8)
        }),
          ...
          for (let e of t.tools)
            if (i[e]) {
              ...
              document
                ...
                .insertAdjacentHTML('beforeend', `<span id="waifu-tool-${e}">${t}</span>`),
